...
public class Uploader {
	...
	// 保存路径
	private String savePysicDir = pathx.webAppPath() ;//"c:\\00" Constant.uploadPathHtmlImage;
	private String saveDir_rltFmt ="uploadf";
	// 文件允许格式
	...
	public void upload() throws Exception {
		...
		try {
			//savepath:uploadf    getfolder()afet uploadf/20150201
			String savedir_relatePath = this.getFolder(this.saveDir_rltFmt);
			
			if (!this.checkFileType(this.originalName)) {
				this.state = this.errorInfo.get("TYPE");
				return;
			}
//ori name xxx.jpg...getname() after == now_name   123456.jpg
			this.fileName = this.getName(this.originalName);
			this.type = this.getFileExt(this.fileName);
			this.url = savedir_relatePath + "/" + this.fileName;

			String physicalPath = this.getPhysicalPath(this.url);
	...
	public void uploadBase64(String fieldName) {
		String savePath = this.getFolder(this.saveDir_rltFmt);
